...
import { CreateNoteInput, FilterQueryInput, UpdateNoteInput, ParamsInput, params } from "./note.schema";
    ...
    try {
        ...
        if (error instanceof Prisma.PrismaClientKnownRequestError) {
            if (error.code === "P2002") {
                throw new TRPCError({
                    code: "CONFLICT",
                    message: "Note with that title already exist"
                });
            }
        }
        throw error;
...
export const findNoteController = async ({paramsInput}:{paramsInput: ParamsInput}) => {
    try {
        ...
        if (!note) {
            throw new TRPCError({
                code: "NOT_FOUND",
                message: "Message with that ID can't be found",
            });
        }
    ...
    try {
        ...
        if (error instanceof Prisma.PrismaClientKnownRequestError) {
            if (error.code = "P2025") {
                throw new TRPCError({
                    code: "CONFLICT",
                    message: "Note with that title is already exists"
                });
            }
        }
        throw error;
    ...
    try {
        ...
        if (error instanceof Prisma.PrismaClientKnownRequestError) {
            if (error.code === "P2025") {
                throw new TRPCError({
                    code: "NOT_FOUND",
                    message: "Note with that ID can't be found"
                });
            }
        }
        throw error;
